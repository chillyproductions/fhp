...
const particleAmount = 00;
var vectorRes = [20, 20];
if (stream) vectorRes = [20, 200];
var animationLoop;
var mouseDown = false;
var firstTime = true;
...
function startListening() {
  canvas.addEventListener("mousedown", ({ offsetX, offsetY }) => {
    ...
    if (stream) mouseDown = true;
    else addWaterCube(row, colm);
...
function loop() {
  if (firstTime) {
    if (stream) createParticleStream();
    else createRandomParticles();
    firstTime = false;
  }
  ...
  animationLoop = setInterval(() => {
    ...
    moveParticles();
    if (document.getElementById("show-particles").checked) drawParticles();
    if (stream) particleStream();
    drawVectors();
...
function drawVectors() {
  for (let row = 0; row < board.length; row += gridHeight / vectorRes[0]) {
    for (
      let colm = 0;
      colm < board[row].length;
      colm += gridWidth / vectorRes[1]
    ) {
      if (board[row][colm] == "wall") {
        ctx.fillStyle = "black";
        ctx.fillRect(
      ...
      drawVector(
        colm + gridWidth / vectorRes[1] / 2,
        row + gridHeight / vectorRes[0] / 2,
        Math.cos(averageParticleAngle),
        Math.sin(averageParticleAngle),
        size
      );
    }
  }
...
function moveParticles() {
  ...
  for (let row = 0; row < gridHeight; row++) {
    for (let colm = 0; colm < gridWidth; colm++) {
      if (board[row][colm] == "wall" || walls[row * gridWidth + colm]) continue;
